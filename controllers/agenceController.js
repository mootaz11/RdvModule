...
try {
    const agences = await agenceModel.find().populate('user');
...
try {
    const agence = await agenceModel.findById(req.params.id).populate('user');
...
exports.addConseiller=async (req,res)=>{
    ...
    .then(async conseiller=>{
            const agence = await agenceModel.findByIdAndUpdate(req.params.idagence,{$push:{conseillers:conseiller}})
           if(agence){
            conseiller.agence=agence._id;
            conseiller.save(updated=>{
            updated &&  res.status(200).json("conseiller added");
            !updated   && res.status(400).json("conseiller existe déjà ");  
            }).catch(err=>{
                return res.status(500).json(err);
               ...
               return res.status(400).json("agence not found ");  
